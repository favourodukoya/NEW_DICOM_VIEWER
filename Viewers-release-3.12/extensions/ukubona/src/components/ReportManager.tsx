...
function TemplatePanel({
  ...
  if (inline) {
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        {creating && (
          <div className="mb-3 rounded-xl border border-[#1e2433] bg-[#0d1117] p-3 space-y-2">
            <input
              className="w-full rounded-lg bg-[#161b26] px-2.5 py-1.5 text-xs text-white placeholder-[#4b5563] outline-none border border-[#1e2433] focus:border-[#3b82f6]/50"
            ...
            />
            <textarea
              className="w-full resize-none rounded-lg bg-[#161b26] px-2.5 py-1.5 text-xs text-white placeholder-[#4b5563] outline-none border border-[#1e2433] focus:border-[#3b82f6]/50"
              rows={4}
            ...
            <div className="flex justify-end gap-1.5">
              <button onClick={() => setCreating(false)} className="rounded-lg px-2.5 py-1 text-[10px] text-[#6b7280] hover:bg-white/5">Cancel</button>
              <button onClick={handleCreate} className="rounded-lg bg-[#3b82f6] px-2.5 py-1 text-[10px] font-medium text-white hover:bg-[#2563eb]">Save</button>
            </div>
          </div>
        )}
              ...
              <div key={tpl.id} className="group rounded-xl border border-transparent hover:border-[#1e2433] hover:bg-[#161b26] px-3 py-2.5 transition">
                <div className="flex items-start justify-between gap-2">
                  <button
                    ...
                    onClick={() => !disabled && handleApply(tpl)}
                    disabled={disabled}
                  >
                    <div className="text-xs font-medium text-[#d1d5db] truncate">{tpl.name}</div>
                    {tpl.category && <div className="mt-0.5 text-[10px] text-[#4b5563]">{tpl.category}</div>}
                  </button>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition flex-shrink-0">
                    <button
                      onClick={() => !disabled && handleApply(tpl)}
                      className="rounded-md bg-[#3b82f6]/15 px-1.5 py-0.5 text-[10px] font-medium text-[#60a5fa] hover:bg-[#3b82f6]/25"
                      disabled={disabled}
                    >
                      Use
                    </button>
                    {isCustom && (
                      <button onClick={() => handleDelete(tpl.id)} className="rounded-md p-1 text-[#4b5563] hover:bg-red-500/15 hover:text-red-400 transition">
                        {Ico.trash}
                      </button>
                    )}
                  </div>
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-40 mt-1.5 w-72 rounded-xl border border-[#1e2433] bg-[#161b26] shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1e2433]">
            ...
            <button onClick={() => setCreating(v => !v)} className="flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-medium text-[#60a5fa] hover:bg-[#3b82f6]/10">
              {Ico.plus} New
            </button>
          </div>
          {creating && (
            <div className="border-b border-[#1e2433] p-3 space-y-2">
              <input className="w-full rounded-lg bg-[#0d1117] px-3 py-1.5 text-xs text-white placeholder-[#4b5563] outline-none" placeholder="Template name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              <input className="w-full rounded-lg bg-[#0d1117] px-3 py-1.5 text-xs text-white placeholder-[#4b5563] outline-none" placeholder="Category" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} />
              <textarea className="w-full resize-none rounded-lg bg-[#0d1117] px-3 py-1.5 text-xs text-white placeholder-[#4b5563] outline-none" rows={3} placeholder="Report text..." value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} />
          ...
          <div className="max-h-60 overflow-y-auto p-2">
            {allTemplates.map(tpl => {
              const isCustom = tpl.id.startsWith('tpl_custom_');
              return (
                <div key={tpl.id} className="group flex items-center justify-between rounded-xl px-3 py-2 hover:bg-white/5 transition">
...
export default function ReportManager({
  ...
  // Responsive: wide = two-column layout
  const [isWide, setIsWide] = useState(window.innerWidth >= 900);
  useEffect(() => {
    const obs = new ResizeObserver(entries => {
      setIsWide(entries[0].contentRect.width >= 900);
    });
      ...
      <div className="flex-1 overflow-hidden">
        {loading ? (
          ...
          <div className={`flex h-full ${isWide ? 'flex-row' : 'flex-col overflow-y-auto'}`}>
            ...
            {isWide && (
              <div className="w-64 flex-shrink-0 border-l border-[#1e2433] overflow-y-auto p-4">
